...
.is_conda <- function() {
  if (requireNamespace("rsyncrosim", quietly = TRUE)) {
    lib_path <- Sys.getenv("ssim_library", unset = "")
    res <- try({
      if (nzchar(lib_path) && exists("ssimLibrary", asNamespace("rsyncrosim"))) {
        lib <- rsyncrosim::ssimLibrary(lib_path)
        rsyncrosim::useConda(lib)
      } else {
        rsyncrosim::useConda()
      }
    }, silent = TRUE)
  ...
  if (nzchar(cp)) return(TRUE)
  home <- tolower(normalizePath(R.home(), winslash = "/", mustWork = FALSE))
  if (grepl("conda", home)) return(TRUE)
  exe <- tolower(normalizePath(commandArgs()[1], winslash = "/", mustWork = FALSE))
  grepl("conda", exe)
}

.RUN_INSTALLER <- .is_conda()

# If NOT in Conda, define a no-op and skip defining anything else
if (!.RUN_INSTALLER) {
  installDependencies <- function(...) invisible(TRUE)
}
...
if (.RUN_INSTALLER) {
  ...
  installDependencies <- function(attach_minimal = c("magrittr","dplyr")) {
    ...
    # -------------------------
    # MaxEnt preflight (rJava + Java + maxent.jar)
    # -------------------------
    check_maxent_prereqs <- function() {
      # 1) rJava load check
      rjava_ok <- FALSE
      if (pkg_is_installed("rJava", lib_path)) {
        rjava_ok <- isTRUE(
          try({
      ...
      # 2) Java/JVM availability (best-effort)
      # If using conda 'openjdk' + 'r-rjava', this typically "just works".
      # We'll probe .jinit() to confirm
      jvm_ok <- FALSE
      if (rjava_ok) {
        jvm_ok <- isTRUE(try({ .jinit(); TRUE }, silent = TRUE))
        if (!jvm_ok) {
          # try setting JAVA_HOME from conda prefix if present
          cp <- Sys.getenv("CONDA_PREFIX", "")
          if (nzchar(cp)) {
            # common locations for conda openjdk on Windows vs. *nix
            cand <- unique(c(
              file.path(cp, "Library", "java"),          # some builds
              file.path(cp, "Library", "bin"),           # jvm on win
              file.path(cp, "bin"),                      # *nix
              file.path(cp)                              # fallback
            ))
            for (p in cand) {
              if (dir.exists(p)) {
                Sys.setenv(JAVA_HOME = p, PATH = paste(p, Sys.getenv("PATH"), sep = .Platform$path.sep))
                jvm_ok <- isTRUE(try({ .jinit(); TRUE }, silent = TRUE))
                if (jvm_ok) break
              }
            }
      ...
      # 3) maxent.jar presence
      # Priority: env var MAXENT_JAR; else dismo/java; else package dir hint
      jar_path <- Sys.getenv("MAXENT_JAR", "")
      if (!nzchar(jar_path)) {
        # typical path if user places jar in dismo extdata
        jar_path <- system.file("java", "maxent.jar", package = "dismo")
      }
      jar_ok <- nzchar(jar_path) && file.exists(jar_path)

      list(
        rjava_ok = rjava_ok,
        jvm_ok = jvm_ok,
        jar_ok = jar_ok,
        jar_path = if (jar_ok) normalizePath(jar_path, winslash = "/", mustWork = FALSE) else ""
      )
    }

    mx <- check_maxent_prereqs()
    message("\n=== MaxEnt preflight ===")
    message("rJava loadable: ", mx$rjava_ok)
    message("JVM available: ", mx$jvm_ok)
    message("maxent.jar found: ", mx$jar_ok, if (mx$jar_ok) paste0(" (", mx$jar_path, ")") else "")
    if (!mx$jar_ok) {
      message("Tip: set MAXENT_JAR to the full path of maxent.jar or copy it to ",
              "system.file('java', package='dismo')/maxent.jar")
    }

    # -------------------------
    # Verification (presence/version only)
    # -------------------------
    message("\n=== Verification ===")
    verify_pkgs <- c(
      conda_pkgs,
    ...
    # --- Attach only a minimal set so piping/pull() work in the next script ---
